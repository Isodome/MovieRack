...
namespace WinMovieRack.Model
{
    public partial class SQLiteConnector
    {
        ...
        public void updateSeenToMovie(int idMovies, DateTime date, string notes)
        {
            SQLiteCommand command = new SQLiteCommand(connection);
            command.CommandText = string.Format("INSERT INTO Seen (Movies_idMovies, Date, Comment) VALUES(@Movies_idMovies, @Date, @Comment)");
            var param = new SQLiteParameter("@Movies_idMovies") { Value = idMovies };
            command.Parameters.Add(param);
            param = new SQLiteParameter("@Date") { Value = date };
            command.Parameters.Add(param);
            param = new SQLiteParameter("@Comment") { Value = notes };
            command.Parameters.Add(param);
            executeCommandThreadSafe(command);

            SQLiteCommand cmdInsert = new SQLiteCommand(connection);
            cmdInsert.CommandText = string.Format("UPDATE Movies SET lastSeen=@lastSeen WHERE idMovies = @idMovies");

            param = new SQLiteParameter("@idMovies") { Value = idMovies };
            cmdInsert.Parameters.Add(param);
            param = new SQLiteParameter("@lastSeen") { Value = date };
            cmdInsert.Parameters.Add(param);
            executeCommandThreadSafe(cmdInsert);
